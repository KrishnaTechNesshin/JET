...
if(isset($_POST["import"]))
{
 ...
 if(in_array($extension, $allowed_extension)) //check selected file extension is present in allowed extension array
 {
  ...
  foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
  {
   ...
   {
    $lesson = mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(0, $row)->getValue());
    $vocabulary = mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(1, $row)->getValue());
    $kanji = mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(2, $row)->getValue());
    $meaning = mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(3, $row)->getValue());
    $status = 1;
    if(!empty($lesson) || !empty($vocabulary) || !empty($meaning)) // if none of the data are empty
    {
      $output .= "<tr>";
      $query = "INSERT INTO vocabulary(lesson,vocabulary,kanji,meaning,status) VALUES ('".$lesson."', '".$vocabulary."', '".$kanji."', '".$meaning."', '".$status."')";
